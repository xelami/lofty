...
import { desktops, desktopMembers, folders, files } from "@lofty/db/schema"

import { and, eq, isNull } from "drizzle-orm"

import { createFileSchema, renameSchema } from "@lofty/validation"
...
import { canEditDesktop, canEditFiles } from "../auth/permissions.js"

async function getMembership(desktopId: string, userId: string) {
  const result = await db
    ...
    .where(
      and(
        ...
        eq(desktopMembers.userId, userId),
      ),
    )
    .limit(1)
...
async function folderBelongsToDesktop(folderId: string, desktopId: string) {
  const result = await db
    .select({
      id: folders.id,
    })
...
export async function fileRoutes(app: FastifyInstance) {
  ...
  app.get("/:desktopId/files", async (request, reply) => {
    ...
    const { folderId } = request.query as {
      folderId?: string
    }
  ...
  app.get("/:desktopId/files/:fileId/download-url", async (request, reply) => {
    ...
    const { desktopId, fileId } = request.params as {
      desktopId: string
      fileId: string
    }

    const { download } = request.query as {
      download?: string
    }
  ...
  app.patch("/:desktopId/files/:fileId", async (request, reply) => {
    ...
    if (body.folderId) {
      const folderExists = await folderBelongsToDesktop(
        body.folderId,
        desktopId,
      )
