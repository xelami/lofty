...
async function getFolder(desktopId: string, folderId: string) {
  const result = await db
    .select()
    .from(folders)
...
async function getDescendantFolderIds(desktopId: string, rootFolderId: string) {
...
export async function folderRoutes(app: FastifyInstance) {
  ...
  app.get("/:desktopId/folders", async (request, reply) => {
    ...
    const { parentFolderId } = request.query as {
    ...
    const result = await db
      ...
      .where(
        parentFolderId
          ? and(
              eq(folders.desktopId, desktopId),
  ...
  app.patch("/:desktopId/folders/:folderId", async (request, reply) => {
    ...
    const { desktopId, folderId } = request.params as {
      desktopId: string
      folderId: string
    }
  ...
  app.delete("/:desktopId/folders/:folderId", async (request, reply) => {
    ...
    const matchingFiles = filesToDelete.filter(
