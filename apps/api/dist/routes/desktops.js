...
import { desktops, desktopMembers, desktopItems, folders, files, } from "@lofty/db/schema";
...
export async function desktopRoutes(app) {
    ...
    app.get("/:id/items", async (request, reply) => {
        ...
        const membership = await db
            ...
            .from(desktopMembers)
            .where(and(eq(desktopMembers.desktopId, id), eq(desktopMembers.userId, request.user.id)))
            .limit(1);
    ...
    app.post("/:id/transfer", async (request, reply) => {
        ...
        const targetMembership = await db
            ...
            .from(desktopMembers)
            .where(and(eq(desktopMembers.desktopId, id), eq(desktopMembers.userId, newOwnerId)))
            .limit(1);
